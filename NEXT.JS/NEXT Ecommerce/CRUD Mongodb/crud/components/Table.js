...
import { toggleChangAction, updateAction, deleteAction } from '../redux/reducer'
...
function Tr({ _id, name, avatar, email, salary, date, status }) {
    ...
    const onUpdate = () => {
        dispatch(toggleChangAction(_id))
        if (visible) {
    ...
    return (
        <tr className='bg-gray-50 text-center'>
            ...
            <td className='px-16 py-2'>
                <button className='cursor'><span className={`${status === "Active" ? 'bg-green-500' : 'bg-rose-500'} text-white px-5 py-1 rounded-3xl`}>{status || 'Unknown'}</span></button>
            </td>
