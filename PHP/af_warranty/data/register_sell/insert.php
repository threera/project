...
$Purchase_date  = date_edit_insert($_POST['Purchase_date']);
...
if ($SerialNumber_Start != "" && $SerialNumber_End != "") {
    if (strAll($cutstr_end, $cutstr1_end) == true) {
        for ($i = $cutstr_end; $i <= $cutstr1_end; $i++) {
            ...
            $query_click = "SELECT * FROM register_sell where SerialNumber = '$SerialNumber_chang'";
            ...
            if ($row_click == 0 && $row_click1) {
                $sql = "INSERT INTO `register_sell`(`Purchase_date`,`Register_date`,`SerialNumber`,`Brand`,`Product`,`Model`, `Warranty`, `Dealer`, `Customer_name`,`Email`, `Tel`, `Number`, `Village`, `Soi`, `Road`, `province`, `amphure`, `district`, `zipcode`) 
    ...
    $query_click = "SELECT * FROM register_sell where SerialNumber = '$SerialNumber'";
    $result_click  = mysqli_query($conn, $query_click);
    $row_click = mysqli_fetch_array($result_click);
    if ($row_click == 0) {
        $sql = "INSERT INTO `register_sell`(`Purchase_date`,`Register_date`,`SerialNumber`,`Brand`,`Product`,`Model`, `Warranty`, `Dealer`, `Customer_name`,`Email`, `Tel`, `Number`, `Village`, `Soi`, `Road`, `province`, `amphure`, `district`, `zipcode`) 
