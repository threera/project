<section id="team" class="team section-bg">
  <div class="container" data-aos="fade-up">
    <div class="section-title mx-auto" style="text-align: start;box-shadow: rgba(0, 0, 0, 0.24) 0px 3px 8px;">
      <h2 class="text-center">ผู้ซื้อ</h2>
      <div class="row">
        <div class="col-lg-12">
          <form id="register_buy" method="POST" class="row  g-3 needs-validation" novalidate>
            ...
            <div class="col-lg-12 m-0" id="SerialNumberALL" style="display: none;">
              <div class="row">
                <p class="text-danger my-2 h5">* ดูวิธีการลงทะเบียนหลายเครื่องโดยคลิกที่เครื่องหมาย ?</p>
            ...
            <hr>
            <div class="col-xl-3 col-md-6 col-sm-12 m-0">
            ...
            <div class="col-xl-3 col-md-6 col-sm-12">
              ...
              <select name="amphure" id="amphure" class="form-select" required>
                <option value="">เลือกอำเภอ</option>
                <?php
                $sql = "SELECT name_th FROM amphures";
                $query = mysqli_query($conn, $sql);
                while ($result = mysqli_fetch_assoc($query)) : ?>
                  <option value="<?= $result['name_th'] ?>"><?= $result['name_th'] ?></option>
                <?php endwhile; ?>
            ...
            <div class="col-xl-3 col-md-6 col-sm-12">
              ...
              <select name="district" id="district" class="form-select" required>
                ...
                <?php
                $sql = "SELECT name_th FROM districts";
                $query = mysqli_query($conn, $sql);
                while ($result = mysqli_fetch_assoc($query)) : ?>
                  <option value="<?= $result['name_th'] ?>"><?= $result['name_th'] ?></option>
                <?php endwhile; ?>
            ...
            <div class="col-12 text-center mt-4 fo-button">
              ...
              <button type="submit" class="btn btn-primary px-5 py-3 disabled" id="register_buy_save">ลงทะเบียน</button>
            </div>
          </form>
        </div>
      </div>
